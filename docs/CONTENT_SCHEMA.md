...
| kind | 表示 | 発生 |
|---|---|---|
| `newcomer` | 転入 | 強制。作業を中断して始まる。**直後に家選び(内見)が始まる回だけ**がこれ |
| `farewell` | 転出 | 強制。作業を中断して始まる。**その人が本当に村(街)を出ていく回だけ** |
| `life` | 人生 | 強制 |
...
**`newcomer` は「直後に家選びが始まる」回にだけ付ける。**引っ越しの話題が出るだけでは転入ではない。

- 「息子夫婦を村に入れたい」と大家が相談に来る → **悩み**(入る家はもう決まっている)
- 「越してきました。家を探しています」→ 案内が始まる → **転入**

`farewell` も同じで、**その人が実際に去る回**にだけ付ける。去る話をしているだけなら悩み。

**転入と転出は、村の空き物件の数を動かす。**

| | 何が起きるか |
|---|---|
| 転入 | 家探しが始まる。契約すると**空きが1つ埋まる** |
| 転出 | その人が住んでいた家が**空きに戻る** |

だから二つは対で設計する。転出が無いと空きが尽きて誰も入れなくなり、転入が無いと村が減る一方になる。
**世代ごとの転入と転出の比が、そのまま村の人口曲線になる。**

### 恋愛も物件選びが始まる

恋愛は `content/gen*/romance/` に置く別系統だが、**ゲーム上は転入と同じで、直後に物件選びが始まる**。
親密度が上がると想い人が「家を見に行きたい」と言い出し、案内すると内見になる。

そのため恋愛コンテンツには、転入世帯と同じ情報が要る。

| 項目 | 中身 |
|---|---|
| `houseInviteLines` | 家を見に行きたいと言い出す台詞 |
| `idealHome.description` | どんな家を求めているか |
| `idealHome.likedFeatures` / `dislikedFeatures` | 内見で反応が変わる条件 |
| `reactions.good` / `bad` / `neutral` | 物件を見たときの反応。`{feature}` が条件名に置き換わる |
| `proposalLines` | 決まったときの台詞 |

**美一〜美五は旅人なので、探すのは一時的な住まい。**契約すれば空きが1つ埋まり、**去るときにまた空きに戻る**。
