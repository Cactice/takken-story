...
> `"mentorId": "umizawa-kyle"` を持たせる(第2世代のように、メンター本人が登場人物でもある場合)。
...
```jsonc
{
  ...
  "descendantOf": "minuma-tanezo", // 任意。先祖のキャラID。子孫関係を機械的に辿るために使う
  "romanceable": false,
...
暦は **主一の生年 1000年が基準**の絶対年。世代内の通し番号ではない。

| 世代 | 舞台 | 主人公の生年 | 世代の開始 |
|---|---|---|---|
| 1 | ありきた村 | 1000年 | 1015年(15歳) |
| 2 | 黒会市 | 1030年 | 1045年 |
| 3 | ありきた村 | 1060年 | 1075年 |
| 4 | 黒会市 | 1090年 | 1105年 |
| 5 | ありきた村 | 1120年 | 1135年 |

1世代は15年。`year` は開始年〜開始年+14 の範囲に収める。
`month` は 1〜12。**この順に読むと物語になる**ように振ること。

### kind — イベントの種類は4つだけ

| kind | 表示 | 発生 |
|---|---|---|
| `newcomer` | 転入 | 強制。作業を中断して始まる |
| `farewell` | 転出 | 強制。作業を中断して始まる |
| `life` | 人生 | 強制 |
| `trouble` | 悩み | 住民に話しかけると始まる。大半はこれ |

恋愛はイベントではない。`content/gen*/romance/` に置き、住民の頭に「!」ではなく♥が出る。強制ではないので断れる。
